...
public class Commands {
    ...
    public static String differentiate(String expression, String variable) {
        ...
        return expr.differentiate(var).toString();
    }
     ...
     */
    public static String simplify(String expression, Map<String,String> environment) {
        Expression expr = Expression.parse(expression);
        Map<Variable, Number> envir = new HashMap<>();
        for (String variable : environment.keySet()) {
        	Number value;
        	if (environment.get(variable).matches("[0-9]+")) {
        		value = new Number(Integer.parseInt(environment.get(variable)));
        	} else {
        		value = new Number(Double.valueOf(environment.get(variable)));
        	}
        	envir.put(new Variable(variable), value);
        }
        return expr.simplify(envir).toString();
    }
